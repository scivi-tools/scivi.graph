...
with open("data_raw.csv") as lines:
    for line in lines:
        if skip > 0:
            ...
            values = line.split(",")
            nodes.append(Node(values[0], values[2], values[5], values[3]))
...
with open("data_links.csv") as lines:
    for line in lines:
        if skip > 0:
            ...
            values = line.split(";")
            src = next(node for node in nodes if node.label == values[0].strip())
            dst = next(node for node in nodes if node.label == values[1].strip())
